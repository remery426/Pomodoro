  To run the application you should just need to have node + npm installed, run npm install inside the root directory, then run npm start to launch the project or npm test to execute tests. Below is the default README created with npx create-react-app, which I have left in for completeness.
